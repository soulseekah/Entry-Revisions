...
class GV_Entry_Revisions {
	 ...
	 */
	private function get_diff( $previous = array(), $current = array(), $form = array(), $restore_links = false ) {
		...
		$r .= '<caption align="top">' . $this->revision_title( $previous, false, esc_html__( 'Edited by %2$s, %3$s ago (%4$s).', 'gravityview-entry-revisions' ) ) . '</caption>';

		$r .= "<colgroup><col class='content diffsplit' style='width: 20%;' /><col class='content diffsplit left' /><col class='content diffsplit middle' /><col class='content diffsplit right' /></colgroup>";

		$r .= '<thead>';

        $r .= sprintf( '<tr class="diff-sub-title">
                <th scope="col" id="diff-col-field" class="screen-reader-text">%s</th>
                <th scope="col" id="diff-col-revision">%s</th>
                <th scope="col" id="diff-col-gap"></th>
                <th scope="col" id="diff-col-current">%s</th>
            </tr>', esc_html__( 'Field', 'gravityview-entry-revision' ), esc_html__( 'Current Values', 'gravityview-entry-revisions' ), esc_html__( 'Edit', 'gravityview-entry-revisions' )
        );
		...
		foreach ( $previous as $key => $previous_value ) {
			...
			$args = apply_filters( 'gravityview/entry-revisions/diff-row-args', array(
                'empty_value' => '<em>' . esc_html__( 'No Value', 'gravityview-entry-revisions' ) . '</em>',
				'row_label' => sprintf( '%s <div class="diff-row-details">%s</div>', $label, sprintf( esc_html__( 'Field ID: %s', 'gravityview-entry-revisions' ), $key ) )
			), compact( "field", "label", "key" ) );

			$diff .= $this->text_diff_row( $current_value, $previous_value, $args );
		...
		if ( $restore_links ) {
            ...
            <tfoot>
                <td></td>
			    <td><a href="' . esc_url( remove_query_arg( 'revision' ) ) . '" class="button button-secondary button-large alignleft" title="' . esc_attr__( 'Cancel and do not modify the current entry', 'gravityview-entry-revisions' ) . '">' . esc_html__( 'Cancel', 'gravityview-entry-revisions' ) .'</a></td>
                <td></td>
                <td><a href="' . $this->get_restore_url( $previous ) .'" class="button button-primary button-large alignright" onclick="return confirm(\''. esc_attr__( 'Are you sure? If you continue, the current entry will be updated with the displayed field values.' ) . '\');">' . esc_html__( 'Restore These Values', 'gravityview-entry-revisions' ) . '</a></td>
            </tfoot>';
		}
	...
	function text_diff_row( $left_string, $right_string, $args = null ) {
		...
		if ( ! class_exists( 'GravityView_Diff_Renderer_Table', false ) ) {
			require( plugin_dir_path( __FILE__ ) . 'class-gravityview-diff-renderer-table.php' );
		}
